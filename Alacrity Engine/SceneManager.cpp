...
SceneManager::SceneManager(): 
	...
	fps(60), isRunning(false), renderer(nullptr) {}
...
bool SceneManager::Initialize(std::string name_, int width_, int height_, RendererType type) {
	...
	case RendererType::OPENGL:
		...
		renderer->CreateAWindow(name_, width_, height_);
		renderer->OnCreate();
	...
	case RendererType::VULKAN:
		...
		VisualProfiler::Instrumentor::Get().BeginSession("Initialize Profile", "ProfileResults/Intialize Results.json");
		renderer->OnCreate();
		VisualProfiler::Instrumentor::Get().EndSession();
		break;
...
void SceneManager::Run() {
	...
	while (isRunning) {
		...
		VisualProfiler::Instrumentor::Get().BeginSession("Scene Render Profile", "ProfileResults/Render Results.json");
		currentScene->Render();
		VisualProfiler::Instrumentor::Get().EndSession();
...
void SceneManager::GetEvents() {
	...
	while (SDL_PollEvent(&sdlEvent)) {
		...
		if(rendererType == RendererType::VULKAN) ImGui_ImplSDL2_ProcessEvent(&sdlEvent);
